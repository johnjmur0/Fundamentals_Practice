...
{
    public class DSA_Tests_Recursion2
    {
        [Fact]
        public void Test_Largest_3_Product()
        {
            ...
            var result = Ch_13_Recursion_2.Largest_3_Product(input);
            Assert.Equal(expected, result);

            input = Enumerable.Range(1, 100000).ToArray();
            expected = 100000L * 99999L * 99998L;

            result = Ch_13_Recursion_2.Largest_3_Product(input);
            Assert.Equal(expected, result);
        }
        ...
        public void Test_Find_Missing()
        {
            ...
            int result = Ch_13_Recursion_2.Find_Missing(input);
            Assert.Equal(expected, result);
        }
        ...
        public void Test_Find_Greatest()
        {
            ...
            int result = Ch_13_Recursion_2.Find_Greatest_N2(randomArr);
            sw.Stop();
            var n2_time = sw.Elapsed;
            Assert.Equal(expected, result);

            sw = Stopwatch.StartNew();
            result = Ch_13_Recursion_2.Find_Greatest_N(randomArr);
            sw.Stop();
            var n_time = sw.Elapsed;
            Assert.Equal(expected, result);

            sw = Stopwatch.StartNew();
            result = Ch_13_Recursion_2.Find_Greatest_NlogN(randomArr);
            sw.Stop();
